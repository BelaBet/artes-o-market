...
const ShippingSection = () => (
  <section className="bg-espresso py-[52px] px-9">
    <div className="max-w-[1320px] mx-auto">
      <div className="text-[0.63rem] tracking-[0.2em] uppercase text-parchment/30 mb-2">Entregamos em todo lugar</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px mt-8">
        {cards.map((c, i) => (
          <div key={i} className="bg-parchment/[0.04] border border-parchment/[0.07] p-6 hover:bg-parchment/[0.07] transition-colors">
            <div className="text-[1.7rem] mb-3">{c.icon}</div>
            <div className="font-display text-[1.12rem] text-parchment mb-2">{c.title}</div>
            <div className="text-[0.76rem] font-light text-parchment/40 leading-[1.7]">{c.desc}</div>
          </div>
        ))}
      </div>
    </div>
