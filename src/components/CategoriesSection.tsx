...
const CategoriesSection = ({ onNavigate }: CategoriesSectionProps) => (
  <section className="py-16 px-9">
    <div className="max-w-[1320px] mx-auto">
      <div className="text-[0.63rem] tracking-[0.2em] uppercase text-terra mb-2">Categorias</div>
      <h2 className="font-display font-normal text-[2.1rem] leading-[1.15]">
        Explore por <em className="italic text-terra">técnica</em>
      </h2>
      <div className="grid grid-cols-4 lg:grid-cols-8 border border-border mt-10">
        {CATEGORIES.map((cat, i) => (
          <div
            key={i}
            onClick={onNavigate}
            className="flex flex-col items-center cursor-pointer overflow-hidden relative aspect-[0.78] border-r border-border last:border-r-0 group"
          >
            <img
              src={IMAGES[cat.img]}
              alt={cat.name}
            ...
            <div className="relative z-[1] flex flex-col items-center justify-end h-full pb-3 gap-1">
              <div className="text-[0.58rem] tracking-[0.12em] uppercase font-semibold text-background text-center drop-shadow-lg">
                {cat.name}
              </div>
            </div>
