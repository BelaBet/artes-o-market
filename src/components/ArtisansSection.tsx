...
import { useArtesaosDestaque } from "@/hooks/useArtesaos";

const ArtisansSection = () => {
  const { artesaos } = useArtesaosDestaque(3);

  if (artesaos.length === 0) return null;
  ...
  <section className="py-14 sm:py-[72px] px-4 md:px-9 bg-espresso">
    <div className="max-w-[1320px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-8 sm:mb-10">
        ...
        <Link
          to="/catalogo"
          className="bg-transparent border-none cursor-pointer font-body text-[0.64rem] sm:text-[0.66rem] tracking-[0.14em] uppercase text-parchment/30 hover:text-gold-light transition-colors self-start sm:self-auto"
        >
          Ver todos →
        </Link>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px">
        {artesaos.map((a) => (
          <div key={a.slug} className="bg-parchment/[0.03] border border-parchment/[0.07] cursor-pointer relative overflow-hidden group hover:bg-parchment/[0.055] transition-colors">
            ...
            <div className="h-[180px] sm:h-[200px] overflow-hidden relative">
              <ImagemComPlaceholder src={a.img ?? undefined} alt={a.name} className="w-full h-full object-cover brightness-[0.55] saturate-[0.68] group-hover:scale-[1.04] transition-transform duration-500" />
