...
const HeroSection = ({ onExplore }: HeroSectionProps) => {
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 min-h-[88vh] relative overflow-hidden">
      {/* Left */}
      <div className="bg-espresso px-8 md:px-[52px] py-[68px] flex flex-col justify-center relative overflow-hidden">
        <div
          className="absolute inset-0 opacity-[0.065] bg-cover bg-center"
          style={{ backgroundImage: `url(${IMAGES.weave})` }}
        />
        <div className="relative z-10">
          <div className="text-[0.64rem] tracking-[0.24em] uppercase text-gold-light font-medium mb-6 animate-[fadeIn_1s_ease_both_0.2s]">
            <span className="inline-block w-6 h-px bg-gold align-middle mr-2.5" />
            Marketplace Artesanal Brasileiro
          </div>
          <h1 className="font-display font-light text-[2.5rem] md:text-[3.5rem] leading-[1.08] text-parchment mb-5 animate-[fadeUp_1s_ease_both_0.35s]">
            Artesanato Brasileiro
            <em className="italic text-gold-light block">para o Mundo</em>
          </h1>
          <p className="text-[0.87rem] font-light leading-[1.85] text-parchment/55 max-w-[380px] mb-8 animate-[fadeUp_1s_ease_both_0.5s]">
          ...
          <div className="flex gap-2.5 animate-[fadeUp_1s_ease_both_0.65s] flex-wrap">
            <button
              onClick={onExplore}
              className="bg-terra text-background border-none px-7 py-3 cursor-pointer font-body font-medium text-[0.71rem] tracking-[0.14em] uppercase hover:brightness-90 hover:-translate-y-px transition-all"
            >
              Explorar Catálogo
            </button>
            <button className="bg-transparent text-parchment border border-parchment/30 px-7 py-3 cursor-pointer font-body font-medium text-[0.71rem] tracking-[0.14em] uppercase hover:border-parchment transition-all">
              Abrir Minha Loja
            </button>
          </div>
          <div className="flex gap-3.5 mt-7 animate-[fadeUp_1s_ease_both_0.78s] flex-wrap">
            {["✈️ 50+ países", "🔒 Checkout seguro", "↩️ 30 dias devolução", "🇧🇷 100% Brasileiro"].map((t, i) => (
              <span key={i} className="flex items-center gap-1.5 text-[0.67rem] text-parchment/45 tracking-[0.05em]">
                {t}
                {i < 3 && <span className="w-px h-4 bg-parchment/15 ml-2" />}
              </span>
            ))}
          </div>
          <div className="flex gap-7 mt-9 pt-7 border-t border-parchment/10 animate-[fadeIn_1s_ease_both_0.9s]">
            {[
              { n: "12k", l: "Artesãos" },
              { n: "85k", l: "Peças" },
              { n: "4.9", l: "Avaliação" },
              ...
              <div key={i}>
                <div className="font-display text-[1.9rem] text-gold-light font-light">{s.n}</div>
                <div className="text-[0.64rem] tracking-[0.12em] uppercase text-parchment/30 mt-0.5">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
